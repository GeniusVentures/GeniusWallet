...
tech-stack:
  ...
  patterns:
    ...
    - "Fill-purposed vs. foreground-purposed tokens are not interchangeable: gw.statusError is tuned as a foreground/icon colour (meant to sit ON a surface), not a fill safe to paint text on top of. gw.foundationError is the token meant for that job."
    ...
    - "Rule of Three crossed: the light-mode amber workaround (GWAppearance.isLight ? #92400E : gw.statusWarning) now has 4 independent occurrences (GWWarningNote, compute_panel.dart, job_steps.dart, toast_widget.dart) -- flagged in-place as the next promotion candidate, not promoted here (that edit lives in lib/theme/, outside this plan's files_modified)."
...
- **The 4th occurrence of the light-mode amber workaround was reused, not promoted.** `toast_widget.dart`'s warning type needed the same `GWAppearance.isLight ? #92400E : gw.statusWarning` pattern already duplicated 3 times elsewhere. Promoting it to a genuine `gw.statusWarning` getter lives in `lib/theme/gw_colors.dart`, outside this plan's `files_modified` — flagged in a ponytail comment as the next thing to actually do, not deferred silently.
...
- **Verification:** Full test suite green at 693 (pre-existing baseline) after the fix; re-confirmed at 705 after Tasks 2-3's new tests.
- **Committed in:** `954930e`

---

**Total deviations:** 3 auto-fixed (2 Rule 1 bugs, 1 Rule 3 blocking fix).
**Impact on plan:** All three were necessary for correctness (two genuine AA/rendering bugs found while doing exactly what the plan asked — auditing every button state in both modes — and one compilation break from a residue-closing signature change). No scope creep beyond what Task 2's own action text explicitly asked for ("check every variant and every state").
...
- The 4th occurrence of the light-mode amber workaround (now in `toast_widget.dart`, joining `GWWarningNote`, `compute_panel.dart`, `job_steps.dart`) is flagged in-place and is a strong candidate for promotion to a genuine `gw.statusWarning` getter in a future `lib/theme/` phase.
...
All created/modified files confirmed present on disk (`toast_widget.dart`, `gw_button.dart`, `swap_settings_drawer.dart`, `23-03-CONTRAST.md`, `23-03-SUMMARY.md`). All six task commits confirmed present in `git log --oneline --all` (`7c97670`, `3ae73f6`, `954930e`, `bf9c0b6`, `fb9294e`, `d14dc83`).
