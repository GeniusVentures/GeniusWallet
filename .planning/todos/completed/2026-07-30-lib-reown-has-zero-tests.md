...
FIXED. `test/reown/` now holds 6 files (`approve_drawer_contract_test.dart`,
`calldata_decoder_test.dart`, `dapp_call_details_test.dart`, `handle_dapp_requests_test.dart`,
`utilities_test.dart`, plus fixtures) totalling roughly 3260 lines — the "zero tests" premise no
longer holds. Residual: `lib/reown/reown_connect_button.dart` and `WalletKitInstance.initOnce()`'s
idempotency have no matching test file, so reown coverage is not exhaustive, only no longer zero.
