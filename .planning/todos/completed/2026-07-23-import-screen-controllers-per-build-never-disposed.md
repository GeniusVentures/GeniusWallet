...
FIXED. `lib/onboarding/existing_wallet/view/import_security_screen.dart:18` is now a
`StatefulWidget`; its `TextEditingController`s are created once in `initState` (`:48-61`) rather
than per build, and `dispose()` (`:63-72`) clears and disposes every one of them, plus the wallet
name controller. Commit `bb6506be`.
