...
FIXED. `lib/navigation/router.dart`'s `/buy` route (`:296`) now builds `BanxaBuyScreen` with
sensible defaults, inside the app's `ShellRoute`. Order history moved to its own `/buy/orders`
route (`:85-89`), reached from the buy screen's "View all" rail, so the CTA and the order list are
no longer the same destination.
