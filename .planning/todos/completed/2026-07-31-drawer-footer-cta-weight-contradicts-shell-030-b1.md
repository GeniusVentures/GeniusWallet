...
FIXED. `lib/submit_job/view/widgets/job_steps.dart:668` and `:685` use
`GWButtonVariant.gradientOutline` for the non-committing "Continue" steps, while `:694` uses the
stronger `GWButtonVariant.gradient` only for "Confirm and pay" — the GNUS-burning commit step —
which is the CTA-weight resolution the todo recommended.
