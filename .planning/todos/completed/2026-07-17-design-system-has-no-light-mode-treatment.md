...
## Closed 2026-09-22 (backlog triage)

STALE. The open question was whether `GWCanvasBackground`'s grain gate and `GWMeshBackground`'s
invariant blob colors were an oversight or a deliberate design choice; that is now written down as
an explicit invariant — `lib/theme/genius_wallet_decorations.dart:169-170` states "Both tokens are
fixed-brand, not appearance-aware, matching every other brand colour", and
`lib/components/effects/gw_mesh_background.dart:111-112` documents `brandPrimary`/`Secondary`/`Tertiary`
as "mode-invariant fixed tokens". The two `GWSwitch` findings this todo also listed as open are not
resolved here — they remain tracked in the still-pending
`2026-07-18-dark-mode-disabled-state-visibility-checkbox-switch.md`, so closing this file does not
drop them.
