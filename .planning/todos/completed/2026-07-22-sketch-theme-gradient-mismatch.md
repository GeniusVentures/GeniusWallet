...
FIXED. `.planning/sketches/themes/default.css:20-21` defines `--brand-cta-a: #0AD89C` and
`--brand-cta-b: #0AAEE6`, which are the same two colors `lib/theme/genius_wallet_gradient.dart:26-31`
uses for `brandCta` (`Color(0xFF0AD89C)` / `Color(0xFF0AAEE6)`). The sketch and the shipped
gradient now agree; the todo's own header already recorded this as resolved.
