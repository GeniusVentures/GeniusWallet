...
FIXED. `lib/dev/dev_tools_bubble.dart:405-421` adds a "Mock txns" dev button that injects a batch
of mock transactions via `DevMockTransactions`, alongside the existing `DevMockJob`
(`lib/dev/dev_mock_job.dart`) and `DevMockSgnus` (`lib/dev/dev_mock_sgnus.dart`) injector classes.
The dev tools bubble's mock-data section now covers more than the original single injector.
