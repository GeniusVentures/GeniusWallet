...
FIXED. The zoom/pan control row that was consuming the chart card's vertical budget no longer
exists — `lib/chart/crypto_live_chart.dart:148` documents that "no zoom/pan controls drive this
anymore (the four buttons were replaced by the timeframe segment, 078-S1)", and the row itself
(previously around `:334-345`) was deleted outright in that refactor.
