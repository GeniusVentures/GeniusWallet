...
FIXED. `lib/utils/secret_clipboard.dart`'s `SecretClipboard`/`scheduleSecretClipboardClear` now
backs both copy sites. `lib/account/sdk_account_manager.dart:310-336` — the sharper edge the todo
called out — shows a confirmation dialog ("Copy recovery phrase?") before writing the mnemonic to
the clipboard, then schedules the clear. `lib/onboarding/new_wallet/view/recovery_phrase_screen.dart:202-203`
schedules the same timed clear; it does not add its own confirm dialog, consistent with the todo's
own reasoning that this screen "sits behind a screen the user knowingly navigated to."
