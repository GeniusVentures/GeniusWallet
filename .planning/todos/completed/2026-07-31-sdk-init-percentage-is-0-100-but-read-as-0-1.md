...
STALE. The todo retracted its own central finding: the `init=37.0` reading that seeded this todo
was traced to `DevMockSgnus.armInitPercentage(37)` (a dev-button press), not a genuine FFI read.
`packages/genius_api/lib/src/genius_api.dart:81` still documents the field as 0.0-1.0, and
`lib/dev/dev_mock_sgnus.dart:129` documents its override as the same 0.0-1.0 scale — nothing here
contradicts that contract anymore.
