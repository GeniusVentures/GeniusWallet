...
FIXED. `lib/tokens/token_info_screen.dart:742-776` now wires a Swap CTA with full
`preselectSymbol`/`preselectChainId` preselection, and no dead Send button remains on the screen.
Residual: `GeniusApi.transferTokens` (`packages/genius_api/lib/src/genius_api.dart:882`) still has
zero callers anywhere in `lib/`, so Send stays a deliberate non-build rather than a broken button
users can tap.
