...
FIXED. `lib/components/buttons/gw_button.dart:305-315` and `:368-407` unify the outer container's
`BorderRadius` and the `InkWell`'s `borderRadius` on the same value, and the app's Material button
themes set `overlayColor: Colors.transparent` globally, so no build-in ink shape competes with the
button's own radius.

One thing worth a separate confirmation: the file's hover comment now reads "Approved hover =
option A · Brighten" as settled fact, but the original todo left this as "TBD — ask Alex" and no
commit or thread in this repo records that approval — the decision appears to have been made
off-repo.
