...
FIXED. `lib/banxa/banxa_api_services.dart:265-304`'s `fetchAllOrders` now loops pages up to
`maxPages = 20` at `pageSize = 100`, a 2,000-order ceiling. The 100-order limit is gone; the new
ceiling is documented in place with a `ponytail:` comment (`:259`) naming both the ceiling and the
upgrade path (a cursor, if Banxa exposes one).
