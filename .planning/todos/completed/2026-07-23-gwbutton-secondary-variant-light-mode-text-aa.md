...
FIXED. `lib/components/buttons/gw_button.dart:136-149`'s secondary variant now reads
`gw.brandPrimaryOnSurface` for both foreground and border, rather than the raw `brandPrimaryStrong`
the todo flagged — the inline comment records the light-mode pairing moving from 1.93:1 to 4.76:1.
The only thing the todo still asked for beyond the code fix is a live visual walk, which was not
part of this triage.
