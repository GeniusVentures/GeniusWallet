...
FIXED. The section-title widget landed as `GWSectionTitle` (`lib/components/cards/gw_section_title.dart`),
not the `GWSectionHeader` name the todo proposed. It is now used by
`lib/dashboard/chart/dashboard_markets.dart`, `lib/dashboard/chart/markets_screen.dart`,
`lib/dashboard/compute/compute_panel.dart`, `lib/dashboard/home/widgets/transactions_slim_view.dart`,
`lib/dashboard/news/view/crypto_news_screen.dart` and `lib/components/coins/view/coins_screen.dart` —
one shared component across the dashboard's section cards instead of each card styling its own title.
