...
FIXED. `.planning/phases/04-navigation-shell-chrome/04-VERIFICATION.md`'s BEH-02 row now states the
`!_dirty`/`GlobalSwapFabHost` sub-clause was "explicitly re-homed to the swap-FAB phase per D-08 —
a documented carry-move, not a silent drop", and `.planning/phases/08-swap-bridge/08-02-PLAN.md`
shows that phase actually porting `GlobalSwapFabHost` and its `_ready`/`!_dirty` gate. The claim was
tracked, not dropped.
