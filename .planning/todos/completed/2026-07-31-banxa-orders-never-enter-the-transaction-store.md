...
STALE. Superseded in practice: `lib/banxa/order_transaction_mapping.dart`'s `orderAsTransaction()`
(`:185`) builds a per-order `Transaction` that the Buy screen renders directly in its own rail and
drawer (`lib/banxa/banxa_buy_screen.dart:1374-1387`), and the "View all" entry point the original
todo hung on was deleted along with it. Residual, recorded honestly: these orders still never reach
`TransactionsCubit`/`TransactionStorageService` (no references to either in `lib/banxa/`), so
whether Banxa orders belong in the app-wide Transactions tab is still an open product question — just
no longer an urgent one blocking anything.
