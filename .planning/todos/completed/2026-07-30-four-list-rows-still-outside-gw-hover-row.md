...
FIXED. All four rows were deliberately kept off `GWHoverRow`, not missed: `job_step_list.dart:161`
uses `GestureDetector` because, per its own comment, a step row is a progress indicator with no
click target, not an interactive row; `sdk_account_manager.dart`'s mode-chip `InkWell` already
carries `borderRadius: radiusPill`; `submit_logs_screen.dart:669` runs its own `StatefulBuilder`-driven
hover state for its tab underline; and `bridge_screen.dart`'s network picker now uses `GWSelectRow`
(`:208`) instead of a bare `ListTile`.
