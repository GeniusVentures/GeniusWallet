...
FIXED. `lib/components/overlay/nav_destinations.dart` now defines a curated `mobileDestinations`
list (Home, Assets, Activity, News — four tabs) plus a centre Swap dock, rendered by
`responsive_overlay.dart`'s bottom bar (commit `d3f1122c`), against the full 8-entry
`allDestinations` desktop set in the same file. The mobile bar no longer mirrors the desktop nav
1:1.
