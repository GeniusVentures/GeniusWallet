...
FIXED. `lib/tokens/token_info_screen.dart` no longer references `CoinCardRow` or
`_MarketDataInfo.topSlot` (`grep -n "CoinCardRow\|_MarketDataInfo" lib/tokens/token_info_screen.dart`
returns no matches) — the screen was rebuilt rather than reusing the A2-redesign row component.
