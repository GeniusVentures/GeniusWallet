...
FIXED. The offending `Row` of four zoom/pan `IconButton`s no longer exists in
`lib/chart/crypto_live_chart.dart` — it was replaced by the timeframe segment control referenced
in `_ChartHeaderRow` during the same refactor documented at `crypto_live_chart.dart:148-150`, so
there is no overflowing row left to walk.
