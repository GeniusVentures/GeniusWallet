...
FIXED. `lib/banxa/banxa_order_cubit.dart:27-30`'s `OrdersCubit._customerId` derives the Banxa
customer key via `banxaCustomerId(_walletDetailsCubit?.state.selectedWallet?.address)` rather than
a literal. `lib/banxa/banxa_api_services.dart:254` documents the old `'your-cust-id'` placeholder
by name as the bug this replaced; no such literal remains in `lib/banxa/`.
