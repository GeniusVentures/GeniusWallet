...
FIXED. The bounded splash widget was replaced by `lib/screens/splash.dart` (commit `29b183b8`),
which composes `Scaffold(body: GWMeshBackground(...))` — a genuinely full-bleed background rather
than a sized box. `lib/screens/boot_sequence.dart` was reduced in the same change to a plain
`class BootSequence` with no `Scaffold` or layout of its own — it now owns only timing, not
rendering.
