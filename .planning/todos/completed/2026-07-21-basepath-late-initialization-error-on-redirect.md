...
FIXED. `packages/genius_api/lib/src/genius_api.dart:189` — `initSDK() => _initFuture ??= _doInitSDK();`
memoizes the init future so a second call while init is in flight returns the same future instead
of racing it. `_initSDK` (`:202-210`) then checks `if (_isSdkInitialized) { return; }` before
`_basePath` is assigned, so a caller can no longer observe `_basePath` in a partially-initialized
state.
