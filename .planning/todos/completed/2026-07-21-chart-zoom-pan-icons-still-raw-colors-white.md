...
FIXED. The same refactor that removed the zoom/pan row (see the companion
`2026-07-21-chart-zoom-pan-row-overflows-34px.md` closure) removed its `Colors.white` `IconButton`s
with it — `grep -n "Colors.white" lib/chart/crypto_live_chart.dart` returns no hits; the file's
remaining `Colors.transparent` uses are unrelated fl_chart sentinel values.
