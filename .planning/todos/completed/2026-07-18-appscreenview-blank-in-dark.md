...
FIXED. `lib/theme/theme.dart:80` wires `textTheme: GeniusWalletTypography.toMaterialTextTheme()`
into the app's `ThemeData`, and `genius_wallet_typography.dart`'s private `_inter()` helper (which
every text style delegates to) defaults its color to `_gw.textPrimary` when none is given. Text no
longer inherits Material's default white in dark mode with no color source.
