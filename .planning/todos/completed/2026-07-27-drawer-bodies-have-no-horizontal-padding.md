...
FIXED. `lib/components/bottom_drawer/responsive_drawer.dart:29-33` defines
`const EdgeInsets kDrawerBodyPadding` (horizontal `space10`) as the drawer body's default
`bodyPadding`, reversing the earlier no-blanket-body-padding stance the todo was filed against. A
caller with a scrolling viewport can still opt out with `EdgeInsets.zero`.
