...
Last activity: 2026-07-21 — **PR #210 MERGED by Jakub.** The end-of-day handoff's blocker (commit/PR gate) is RESOLVED: the 7 walked-and-approved quick tasks (uhe/vwj/0ze/1nk/bxr/ch1/ed9) plus 2 more from session 2 (baz, dws) landed in `0bcf3df` (29 files, +1648/-415); docs + sketches 001–008 landed in `73a09a4`. Navbar = FINAL. Working tree clean, local == origin.
...
1. **Gap 2 — dashboard retry.** ROADMAP criterion 3 ("with a working retry") vs UI-SPEC §6 (locks develop's string, forbids `GWErrorState`). Implement a retry, or record an override rewording the criterion.
2. **UI-SPEC §3.1** still carries the 1.96:1 pairing. Correct the contract.
