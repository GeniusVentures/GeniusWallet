...
- **develop's `theme.dart` is NOT appearance-aware and Phase 4 must wire it** (2026-07-17) — zero `GWAppearance` references; Phase 2 deferred it (UI-SPEC §1.1 excludes `theme.dart` wholesale as a 100%-collision file). First real consequence hit in 03-09: the gallery's faithfully-ported `Scaffold(backgroundColor: Colors.transparent)` — which relies on that appearance-aware theme in Alex's app — fell through to develop's permanently-dark theme while `textPrimary` flipped to near-black ink, making light mode unreadable. Worked around in the dev-only gallery (`244b71e`, bound to `surfaceBase`, matching Phase 2's token probe); **revert that line to `Colors.transparent` when Phase 4 lands the real theme.** Expect the same class of surprise on any Phase 4+ screen that assumes a flipping theme
